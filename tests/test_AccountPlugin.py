...
async def account_init(contract_classes):
    ...
    return starknet.state, account1, account2, initializable1, initializable2, attacker, ECDSA_plugin_class_hash, Schnorr_class_hash


@pytest.fixture
def account_factory(contract_classes, account_init):
    account_cls, init_cls, attacker_cls, ECDSA_plugin_cls, Schnorr_cls = contract_classes
    state, account1, account2, initializable1, initializable2, attacker, ECDSA_plugin_class_hash, Schnorr_class_hash = account_init
    _state = state.copy()
    ...
    return ECDSA_plugin_class_hash, account1, account2, initializable1, initializable2, attacker, Schnorr_class_hash
...
async def test_schnorr(account_factory):
    ...
    await schnorrSigner.send_transactions(account,
        [
            (account.contract_address, 'use_plugin', [Schnorr_class_hash]),
            (initializable_1.contract_address, 'initialize', [])
        ]
    )
